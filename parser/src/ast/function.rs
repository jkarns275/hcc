...
impl Function {
    pub fn from_pair<'r>(pair: Pair<'r, Rule>, context: &mut Context<'r>)
        ...
        let (name, args) = Function::name_and_args_from_pair(fn_declarator, context)?;
    ...
    fn name_and_args_from_pair<'r>(pair: Pair<'r, Rule>, context: &mut Context<'r>)
        -> Result<(Id, HashMap<Id, Declaration>), AstError> {
        ...
        let declarator_call: Pair<'r, Rule> = expect!(pairs, Rule::direct_declarator_call, "direct_declarator_call", span);
