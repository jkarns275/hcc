...
impl CG {
    ...
    pub fn generate_method_header(&mut self, it: &Function, conforming_structure: Id, structure: Id) -> String {
        ...
        let method_name = format!("{}_{}_{}_{}", self.ids.get_str(conforming_structure), self.ids.get_str(structure), 
                                    self.ids.get_str(it.name), it.signature(&self.ids));
    ...
    pub fn visit_struct(&mut self, it: &mut Structure) {
        ...
        for (field_name, field) in it.fields.iter() {
            self.emit(format!("    {} _{};\n", field.ty.to_code(&self.ids), self.ids.get_str(*field_name)));
        }
    ...
    fn visit_stmt(&mut self, it: &mut Statement) -> Label {
        match it {
            ...
            Statement::Expr(ref mut expr)          => { self.visit_expr(expr); Label(0) },
    ...
    fn visit_declaration(&mut self, it: &mut Declaration) -> Label {
        ...
        if let Some(ref mut initializer) = it.initializer.as_mut() {
            ...
            self.emit(format!("    {} _{} = {};\n", it.ty.to_code(&self.ids), self.ids.get_str(it.name), self.ids.get_str(val)));
        } else {
            self.emit(format!("    {} _{};\n", it.ty.to_code(&self.ids), self.ids.get_str(it.name)));
        }
        Label(0)
    }
    ...
    fn visit_mulexpr(&mut self, it: &mut MulExpr) -> Id {
        ...
        self.emit(format!("    {} {} = {};\n", self.current_ty.clone().to_code(&self.ids),  self.ids.get_str(result), exp));
        result
    }
    ...
    fn visit_inverseexpr(&mut self, it: &mut Expr) -> Id {
        ...
        self.emit(format!("    {} {} = ~{};\n", self.current_ty.to_code(&self.ids), self.ids.get_str(result), self.ids.get_str(operand)));
        result
    }
        ...
        for i in (0..it.tail.len() - 1).rev() {
            ...
            match &mut it.tail[i].1.expr {
                ExprKind::Dot(ref mut exp) => {
                    ...
                    self.emit(format!("    {}.{} {} {};\n", self.ids.get_str(lhs), self.ids.get_str(exp.field_name), op, self.ids.get_str(value)));
                },
                ExprKind::Index(ref mut exp) => {
                    let index = self.visit_expr(&mut exp.offset);
                    let base = self.visit_expr(&mut exp.base);
                    self.emit(format!("    {}[{}] {} {};\n", self.ids.get_str(base), self.ids.get_str(index), op, self.ids.get_str(value)));
                },
                ExprKind::Deref(ref mut exp) => {
                    let ptr = self.visit_expr(exp);
                    self.emit(format!("    *{} {} {};\n", self.ids.get_str(ptr), op, self.ids.get_str(value)));
                },
                ExprKind::Ident(ref mut id) => {
                    let name = format!("_{}", self.ids.get_str(*id));
                    self.emit(format!("    {} {} {};\n", name, op, self.ids.get_str(value)));
                },
                _ => panic!("Fix the type checker for assignments dumbo."),
            };
        }
        let val = self.visit_expr(&mut it.tail[0].1);
        self.emit(format!("    {} {} = {};\n", self.current_ty.to_code(&self.ids), self.ids.get_str(result), self.ids.get_str(val)));
        result
    }
    fn visit_leaexpr(&mut self, it: &mut Expr) -> Id {
        ...
        match &mut it.expr {
            ExprKind::Dot(ref mut exp) => {
                let lhs = self.visit_expr(&mut exp.lhs);
                self.emit(format!("    {} {} = &({}.{});\n", self.current_ty.to_code(&self.ids), self.ids.get_str(result), self.ids.get_str(lhs), self.ids.get_str(exp.field_name)));
            },
            ExprKind::Index(ref mut exp) => {
                let index = self.visit_expr(&mut exp.offset);
                let base = self.visit_expr(&mut exp.base);
                self.emit(format!("    {} {} = {}[{}];\n", self.current_ty.to_code(&self.ids), self.ids.get_str(result), self.ids.get_str(base), self.ids.get_str(index)));
            },
            ExprKind::Deref(ref mut exp) => {
                let ptr = self.visit_expr(exp);
                self.emit(format!("    {} {} = {};\n", self.current_ty.to_code(&self.ids), self.ids.get_str(result), self.ids.get_str(ptr)));
            },
            ExprKind::Ident(ref mut id) => {
                let name = format!("_{}", self.ids.get_str(*id));
                self.emit(format!("    {} {} = &{};\n", self.current_ty.to_code(&self.ids), self.ids.get_str(result), name));
            },
    ...
    fn visit_cast(&mut self, it: &mut Cast) -> Id {
        ...
        self.emit(format!("    {} {} = ({}) {};\n", self.current_ty.to_code(&self.ids), self.ids.get_str(result), it.to.to_code(&self.ids), self.ids.get_str(value)));
        result
    }
    ...
    fn visit_number(&mut self, it: &mut i64) -> Id {
        let result = self.next_tmp();
        self.emit(format!("    {} {} = {:#x};\n", self.current_ty.to_code(&self.ids), self.ids.get_str(result), *it));
        result
    }
    fn visit_new(&mut self, it: &mut Ty) -> Id {
        ...
        if it.ptr == 0 {
            ...
            self.emit(format!("{0}* {1} = ({0}*) malloc(sizeof (void*));\n", it.to_code(&self.ids), self.ids.get_str(binding)));
        }
        binding
    }

    fn visit_method(&mut self, it: &mut Function, conforming_structure: Id, structure: Id) -> Label {
        let result = self.next_tmp();
